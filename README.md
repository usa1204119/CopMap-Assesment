...
### Critical Gaps (Not Implemented - Blockers for Production)

| Feature | Status | Why Skipped | Time to Implement |
|---------|--------|------------|-------------------|
| **Background Location Tracking** | ❌ | Requires Android/iOS-specific services | 4-6 hours |
| **Firestore Security Rules** | ❌ | Critical but not urgent for beta | 2-3 hours |
| **Push Notifications** | ❌ | Firebase Messaging setup needed | 3-4 hours |
| **API Key Management** | ❌ | Currently hardcoded (security issue) | 1-2 hours |
| **Unit Tests** | ❌ | Requires test infrastructure | 6-8 hours |
| **Integration Tests** | ❌ | Complex Firebase mocking needed | 4-5 hours |

### Deferred Features (Medium Priority)

| Feature | Status | Reason | Alternative |
|---------|--------|--------|-------------|
| **Geofencing** | ⏳ | Complex implementation | Manual boundary checking |
| **Offline Support** | ⏳ | Requires local DB sync | Cloud-first approach |
| **Emergency SOS** | ⏳ | Low priority for initial MVP | Standard alert system works |
| **Advanced Reporting** | ⏳ | Analytics not critical yet | Manual exports from Firestore |
| **Multi-language Support** | ⏳ | English-only for MVP | Can add intl package later |
| **Marker Clustering** | ⏳ | Low performance need now | Add when 100+ officers |

---

## 🔀 Trade-offs & Decisions

### 1. **Real-Time vs Cost Efficiency**

**Decision**: Use Firestore listeners for real-time updates

**Pros:**
- ✅ Live updates with <1 second latency
- ✅ Automatic synchronization
- ✅ Firebase handles scaling

**Cons:**
- ❌ Higher Firestore costs (more read operations)
- ❌ Continuous network usage
- ❌ Battery drain on mobile

**Alternative Considered**: REST API with polling (rejected - too slow)

---

### 2. **Cross-Platform vs Native Performance**

**Decision**: Flutter for all platforms (Web + Android + iOS)
...
- ❌ Larger app bundle size (~50MB)
- ❌ Background services more complex

**Alternative Considered**: Native Android + React Web (rejected - more maintenance)

---

### 3. **Foreground vs Background Location Tracking**

**Current**: Foreground only (quick MVP)

**Decision Rationale:**
- 🎯 Faster to market
- 🎯 Simpler implementation
- 🎯 Lower battery drain for testing

**Planned Improvement**: Background service after MVP validation

---

### 4. **Cloud-First vs Offline-First**

**Decision**: Cloud-first architecture (minimal offline support)
...
**Future Plan**: Add local cache layer with Hive for offline queuing

---

### 5. **Role-Based Access vs Attribute-Based**

**Decision**: Role-based (Station Master vs Field Officer)

**Pros:**
- ✅ Simple to understand
- ✅ Easy to implement Firestore rules
- ✅ Clear permissions model

**Cons:**
- ❌ Cannot have mixed roles
- ❌ Limited granularity

**Future Extension**: Move to ABAC if more roles needed (Supervisor, Administrator, etc.)

---

### 6. **Location Precision vs Battery Life**

**Decision**: 5-meter threshold for location updates

**Rationale:**
- 📍 Sufficient for street-level accuracy
- 🔋 Reduces battery drain from GPS polling
- 📊 Fewer Firestore writes (cost optimization)

**Trade-off**: Slightly delayed map updates vs battery efficiency

---

### 7. **Google Maps vs Open Street Maps**

**Decision**: Google Maps (proprietary)
...
**Alternative**: OpenStreetMap (rejected - inferior UX for this use case)

---

## 🔐 Security Considerations

### ⚠️ Current Vulnerabilities (CRITICAL)

1. **Exposed API Keys**
   - Google Maps key in AndroidManifest.xml
   - Firebase config hardcoded in main.dart
   - Should use google-services.json only

2. **Missing Firestore Rules**
   - Database fully readable/writable
   - Need role-based security rules
   - Officers can modify other officers' data

3. **No Data Encryption**
   - Location data not encrypted
   - Should enable Firestore encryption at rest
...
service cloud.firestore {
  match /databases/{database}/documents {
    // Officers can only read/write their own data
    match /officers/{officerId} {
      allow read, write: if request.auth != null && 
        request.auth.uid == officerId;
    }
    
    // Station masters can read all officers
    match /officers/{officerId} {
      allow read: if request.auth != null && 
        get(/databases/$(database)/documents/users/$(request.auth.uid)).data.role == 'station_master';
    }
    
    // Duties - station masters create, officers read
    match /duties/{dutyId} {
    ...
    // Alerts - readable by all, writable by officers
